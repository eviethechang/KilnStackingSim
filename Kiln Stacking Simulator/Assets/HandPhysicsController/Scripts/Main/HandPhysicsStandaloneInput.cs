...
namespace HandPhysicsExtenstions
{
    ...
    public class HandPhysicsStandaloneInput : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                print(Input.GetAxis(MoveForearmAxisY) + .15f);
                Controller.MoveForearm(new Vector3(3*Input.GetAxis(MoveForearmAxisX), Input.GetAxis(MoveForearmAxisY)+.15f,
                    Input.GetAxis(MoveForearmAxisZ)*3));
            }
            else
            {
